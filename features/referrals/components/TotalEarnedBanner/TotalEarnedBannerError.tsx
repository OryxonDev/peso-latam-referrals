  ...
  return (
    <div className="mb-8 bg-red-50 border border-red-200 rounded-lg p-6">
      <h3 className="text-lg font-semibold text-red-800 mb-3">
        {t('totalEarned.error.title')}
      </h3>
      <p className="text-md text-red-700 mb-2 font-semibold">
        {errorInfo.errorType}
      </p>
      <p className="text-sm text-red-700 mb-2">
        <span className="font-medium">{t('totalEarned.error.whatFailed')}</span> {errorInfo.whatFailed}
      </p>
      <p className="text-sm text-red-700 mb-4">
        <span className="font-medium">{t('totalEarned.error.howToRecover')}</span> {errorInfo.howToRecover}
      </p>
      {refetch && (
        <button
          onClick={handleRetry}
